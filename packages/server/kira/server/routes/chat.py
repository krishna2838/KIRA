...
from kira.prompt_guard import sanitize_user_input, wrap_tool_output
...
async def chat(request: ChatRequest, req: Request):
    ...
    # Fold in document chunks when the memory hits are thin OR the query
    # clearly asks about "my notes/files/pdf/doc/…".
    doc_hits: list[dict] = []
    doc_store = getattr(req.app.state, "document_store", None)
    should_pull_docs = (
        doc_store is not None
        and (len(memories) < 3 or _looks_docish(request.message))
    )
    if should_pull_docs:
        try:
            doc_hits = await doc_store.search(request.message, limit=5)
        except Exception:
            doc_hits = []
    ...
    # Confidence pass — fast model rates how well the reply matches the
    # sources it was allowed to see. Cheap, opt-out if the fast model errors.
    ...
    try:
        ...
        sources_for_grader = memories_text
        confidence_info = await score_confidence(
            response_text, sources_for_grader, _fast_gen
        )
