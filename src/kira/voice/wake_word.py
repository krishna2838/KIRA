...
class WakeWordDetector:
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            # Prefer onnx (onnxruntime is installed; tflite-runtime usually
            # isn't on macOS). Ensure the model files are present.
            framework = "onnx"
            try:
                from openwakeword.utils import download_models
                download_models()
            except Exception:
                pass  # models may already be cached; Model() will tell us

            try:
                self._model = Model(
                    wakeword_models=self.models, inference_framework=framework
                )
            except TypeError:
                # Older openWakeWord without the kwarg.
                self._model = Model(wakeword_models=self.models)
            logger.info(f"Wake word ready: {self.models} ({framework})")
            return True
